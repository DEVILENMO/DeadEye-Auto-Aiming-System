...
"""
原生 TensorRT .trt 推理（需安装 tensorrt、pycuda）。
.engine（Ultralytics 导出）请在 YoloDetector 中走 ultralytics.YOLO，无需加载本模块。
"""
import os
...
class Yolov8TensorRTEngine(object):
    """仅用于 .trt；依赖在 __init__ 内惰性导入，避免未安装 pycuda 时影响 .pt / .engine。"""

    def __init__(self, model_path):
        _, ext = os.path.splitext(model_path)
        if ext.lower() != '.trt':
            raise ValueError(f'Yolov8TensorRTEngine 仅支持 .trt，收到: {ext}')
